...
for (i in 1:length(num)) {
  url = str_c("http://transcript.duma.gov.ru/search/?by=date&sessid=4716&doctype=0&dt_start=&dt_end=&phrase1=&PAGEN_1=", num[i])
  page = read_html(url)
  ...
  duma73 = rbind(duma73, data.frame(name,link))
  i = i + 1
  Sys.sleep(0.05)
}
